...
!function (t) {
    var e = -1, a = -1, o = function (t) {
        return parseFloat(t) || 0
    }, i = function (e) {
        var a = t(e), i = null, n = [];
        return a.each(function () {
            var e = t(this), a = e.offset().top - o(e.css("margin-top")), r = n.length > 0 ? n[n.length - 1] : null;
    ...
    r.version = "master", r._groups = [], r._throttle = 80, r._maintainScroll = !1, r._beforeUpdate = null, r._afterUpdate = null, r._rows = i, r._parse = o, r._parseOptions = n, r._apply = function (e, a) {
        var s = n(a), h = t(e), c = [h], l = t(window).scrollTop(), p = t("html").outerHeight(!0),
        ...
        t("[data-match-height], [data-mh]").each(function () {
            var a = t(this), o = a.attr("data-mh") || a.attr("data-match-height");
    ...
    a.fn.niceSelect = function (d) {
        if (typeof d == "string") {
            if (d == "update") {
                this.each(function () {
                    var f = a(this);
                    var g = a(this).next(".nice-select");
                    var e = g.hasClass("open");
                    if (g.length) {
                        g.remove();
                        c(f);
                        if (e) {
        ...
        function c(g) {
            g.after(a("<div></div>").addClass("nice-select").addClass(g.attr("class") || "").addClass(g.attr("disabled") ? "disabled" : "").addClass(g.attr("multiple") ? "has-multiple" : "").attr("tabindex", g.attr("disabled") ? null : "0").html(g.attr("multiple") ? '<span class="multiple-options"></span><div class="nice-select-search-box"><input type="text" class="nice-select-search" placeholder="Search..."/></div><ul class="list"></ul>' : '<span class="current"></span><div class="nice-select-search-box"><input type="text" class="nice-select-search" placeholder="Search..."/></div><ul class="list"></ul>'));
            var h = g.next();
            var e = g.find("option");
            if (g.attr("multiple")) {
                var f = g.find("option:selected");
                var i = "";
                f.each(function () {
                    $selected_option = a(this);
                    $selected_text = $selected_option.data("display") || $selected_option.text();
                    i += '<span class="current">' + $selected_text + "</span>"
                });
                $select_placeholder = g.data("placeholder") || g.attr("placeholder");
                $select_placeholder = $select_placeholder == "" ? "Select" : $select_placeholder;
                i = i == "" ? $select_placeholder : i;
                h.find(".multiple-options").html(i)
            } else {
                var f = g.find("option:selected");
            ...
            e.each(function (j) {
                var l = a(this);
                var k = l.data("display");
                h.find("ul").append(a("<li></li>").attr("data-value", l.val()).attr("data-display", (k || null)).addClass("option" + (l.is(":selected") ? " selected" : "") + (l.is(":disabled") ? " disabled" : "")).html(l.text()))
            })
        }

        a(document).off(".nice_select");
        a(document).on("click.nice_select", ".nice-select", function (e) {
            var f = a(this);
            a(".nice-select").not(f).removeClass("open");
            f.toggleClass("open");
            if (f.hasClass("open")) {
                f.find(".option");
                f.find(".nice-select-search").val("");
                f.find(".nice-select-search").focus();
                f.find(".focus").removeClass("focus");
        ...
        a(document).on("click", ".nice-select-search-box", function (e) {
            e.stopPropagation();
            return false
        });
        ...
        a(document).on("click.nice_select", ".nice-select .option:not(.disabled)", function (e) {
            var g = a(this);
            var f = g.closest(".nice-select");
            if (f.hasClass("has-multiple")) {
                ...
                $selected_html = "";
                $selected_values = [];
                f.find(".selected").each(function () {
                    $selected_option = a(this);
                    var i = $selected_option.data("display") || $selected_option.text();
                    $selected_html += '<span class="current">' + i + "</span>";
                    $selected_values.push($selected_option.data("value"))
                });
                $select_placeholder = f.prev("select").data("placeholder") || f.prev("select").attr("placeholder");
                $select_placeholder = $select_placeholder == "" ? "Select" : $select_placeholder;
                $selected_html = $selected_html == "" ? $select_placeholder : $selected_html;
                f.find(".multiple-options").html($selected_html);
                f.prev("select").val($selected_values).trigger("change")
            } else {
                f.find(".selected").removeClass("selected");
                g.addClass("selected");
                var h = g.data("display") || g.text();
                f.find(".current").text(h);
                f.prev("select").val(g.data("value")).trigger("change")
            }
        });
        a(document).on("keydown.nice_select", ".nice-select", function (g) {
            var h = a(this);
            var i = a(h.find(".focus") || h.find(".list .option.selected"));
            if (g.keyCode == 32 || g.keyCode == 13) {
                ...
                if (g.keyCode == 40) {
                    if (!h.hasClass("open")) {
                        ...
                        var e = i.nextAll(".option:not(.disabled)").first();
                        if (e.length > 0) {
                            h.find(".focus").removeClass("focus");
                            e.addClass("focus")
                        }
                    }
                    return false
                    ...
                    if (g.keyCode == 38) {
                        if (!h.hasClass("open")) {
                            ...
                            var f = i.prevAll(".option:not(.disabled)").first();
                            if (f.length > 0) {
                                h.find(".focus").removeClass("focus");
                                f.addClass("focus")
                            }
        ...
        a(document).on("keydown.nice-select-search", ".nice-select", function () {
            var h = a(this);
            var g = h.find(".nice-select-search").val();
            var e = h.find("ul li");
            if (g == "") {
                e.show()
            } else {
...
if (CSS.supports("-moz-transform-style", "preserve-3d") || CSS.supports("-ms-transform-style", "preserve-3d") || CSS.supports("-webkit-transform-style", "preserve-3d") || CSS.supports("transform-style", "preserve-3d")) {
    ...
    function zDraw(e, t) {
        if ("false" !== (t.zEngaged || z_default.zEngaged)) {
            var n = t.depth || z_default.depth, r = n.match(/[a-z]+/)[0], a = parseFloat(n.replace(r, "")),
                s = t.direction || z_default.direction, o = t.event || z_default.event,
                l = t.eventRotation || z_default.eventRotation, d = l.match(/[a-z]+/)[0],
                c = parseFloat(l.replace(d, "")), p = t.eventDirection || z_default.eventDirection,
                f = t.fade || z_default.fade, u = t.layers || z_default.layers,
                v = t.perspective || z_default.perspective, y = t.transform || z_default.transform, w = e.innerHTML;
            ...
            if (e.append(z), "pointer" === o && (window.addEventListener("mousemove", e => {
                ...
                scroll(), window.addEventListener("scroll", (function () {
                    var t = e.getBoundingClientRect(), n = t.left + t.width / 2 - window.innerWidth / 2,
                        r = t.top + t.height / 2 - window.innerHeight / 2;
                    g(n / window.innerWidth * -2, r / window.innerHeight * -2)
                }), !1)
            }
            if ("scrollY" == o) {
                scrollY(), window.addEventListener("scroll", (function () {
                    var t = e.getBoundingClientRect();
                    g(0, (t.top + t.height / 2 - window.innerHeight / 2) / window.innerHeight * -2)
                }), !1)
            }
            if ("scrollX" == o) {
                scrollX(), window.addEventListener("scroll", (function () {
                    var t = e.getBoundingClientRect();
